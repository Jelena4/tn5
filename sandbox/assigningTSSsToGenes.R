...
cds.coords = cds.coords %>% mutate(center = (start + end)/2) %>% mutate(length = abs(end - start))
  
utrs = utrs[utrs[,'name'] %in% (cds.coords[,'name'] %>% as.list)$name,]

cds.coords = cds.coords %>% as.data.frame()
...
findTSSsWithinRange = function(range, bed.ranges, utrs.5prime.ranges, bed, assigned.peaks){
  ...
  olaps <<- findOverlaps(bed.ranges, utrs.5prime.ranges, ignore.strand = F, maxgap = range)
  peaks <<- bed[as.data.frame(olaps)$queryHits,]
  peaks <<- peaks %>% mutate(name = utrs.5prime[as.data.frame(olaps)$subjectHits, 'name'])
...
n = 50
findTSSsWithinRange(n, bed.ranges, utrs.5prime.ranges, bed, assigned.peaks)
findTSSsWithinRange(n, bed.ranges, utrs.5prime.ranges, bed, assigned.peaks)
findTSSsWithinRange(n, bed.ranges, utrs.5prime.ranges, bed, assigned.peaks)
findTSSsWithinRange(n, bed.ranges, utrs.5prime.ranges, bed, assigned.peaks)
findTSSsWithinRange(n, bed.ranges, utrs.5prime.ranges, bed, assigned.peaks)
...
assigned.peaks %>% nrow
assigned.peaks %>% ungroup %>% select(V1,V2,V3) %>% n_distinct

#assigned.peaks.forward = assigned.peaks
assigned.peaks.reverse = assigned.peaks
